...
public class OnlineMapsFindDirection : OnlineMapsGoogleAPIQuery
{
    ...
    private OnlineMapsFindDirection(string origin, string destination, bool alternatives = false)
    {
        ...
        url.AppendFormat("https://maps.googleapis.com/maps/api/directions/xml?origin={0}&destination={1}&sensor=false&mode=walking", OnlineMapsWWW.EscapeURL(origin), OnlineMapsWWW.EscapeURL(destination));
        if (alternatives) url.Append("&alternatives=true");
        url.Append("&key=AIzaSyDr9fKgdxvzkglcmIrh4nA_-tcjFjjhYVQ");
