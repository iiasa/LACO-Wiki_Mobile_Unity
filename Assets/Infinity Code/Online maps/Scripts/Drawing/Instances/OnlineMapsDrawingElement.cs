...
public class OnlineMapsDrawingElement: IOnlineMapsInteractiveElement
{
        ...
        lock (points)
        {
            foreach (object p in points)
            {
                ...
                if (drawPart)
                {
                    ...
                    else DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p1tx, p1ty, p2tx, p2ty, i, maxX, ref ppx1, w, invertY, zoomScale);
                }
    ...
    protected List<Vector2> GetLocalPoints(IEnumerable points, bool closed = false, bool optimize = true)
    {
        ...
        int zoom = map.zoom;
        float zoomScale = 1 - (map.floatZoom - zoom) / 2;
        ...
        double ppx = 0;
        double scaleX = OnlineMapsUtils.tileSize * OnlineMapsControlBaseDynamicMesh.instance.sizeInScene.x / map.buffer.renderState.width / zoomScale;
        double scaleY = OnlineMapsUtils.tileSize * OnlineMapsControlBaseDynamicMesh.instance.sizeInScene.y / map.buffer.renderState.height / zoomScale;
    ...
    protected void InitLineMesh(IEnumerable points, OnlineMapsTileSetControl control, ref List<Vector3> vertices, ref List<Vector3> normals, ref List<int> triangles, ref List<Vector2> uv, float width, bool closed = false, bool optimize = true)
    {
        ...
        float sizeX = control.sizeInScene.x / zoomScale;
        float sizeY = control.sizeInScene.y / zoomScale;
