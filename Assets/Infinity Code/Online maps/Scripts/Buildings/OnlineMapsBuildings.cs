...
[Serializable]
[OnlineMapsPlugin("Bing Maps Buildings", typeof(OnlineMapsControlBaseDynamicMesh))]
public class OnlineMapsBuildings : MonoBehaviour, IOnlineMapsSavableComponent
{
    ...
    {
        if (Math.Abs(building.initialZoom - map.floatZoom) < float.Epsilon) building.transform.localScale = Vector3.one;
        else if (building.initialZoom < map.floatZoom) building.transform.localScale = Vector3.one * Mathf.Pow(2, map.floatZoom - building.initialZoom);
        else if (building.initialZoom > map.floatZoom) building.transform.localScale = Vector3.one / Mathf.Pow(2, building.initialZoom - map.floatZoom);
    }
