...
public class OnlineMapsXML : IEnumerable
{
    ...
    public T Get<T>(XmlElement el, T defaultValue)
    {
        ...
        try
        {
            MethodInfo method = OnlineMapsReflectionHelper.GetMethod(underlyingType, "Parse", new[] { typeof(string), typeof(CultureInfo) });
            if (method != null) obj = (T)method.Invoke(null, new object[] { value, CultureInfo.InvariantCulture });
        }
