...
namespace InfinityCode.OnlineMapsDemos
{
    ...
    public class Demo : MonoBehaviour
    {
        ...
        {
            if (string.IsNullOrEmpty(googleAPIKey)) Debug.LogWarning("Please specify Map / Demo (Script) / Google API Key");
        }
        ...
        private void OnGUI()
        {
            ...
            if (GUI.Button(new Rect(5, 60, 50, 50), "+")) map.zoom++;

            Color defBackgroundColor = GUI.backgroundColor;

            for (int i = OnlineMaps.MAXZOOM; i > 2; i--)
            {
                if (map.zoom == i) GUI.backgroundColor = Color.green;
                if (GUI.Button(new Rect(5, 115 + (OnlineMaps.MAXZOOM - i) * 15, 50, 10), "")) map.zoom = i;
                GUI.backgroundColor = defBackgroundColor;
            }

            if (GUI.Button(new Rect(5, 390, 50, 50), "-")) map.zoom--;
            ...
            map.labels = GUI.Toggle(new Rect(200, 50, 100, 30), map.labels, "Labels");
            map.traffic = GUI.Toggle(new Rect(300, 50, 100, 30), map.traffic, "Traffic");
            OnlineMapsElevationManagerBase.instance.enabled = !is2D && GUI.Toggle(new Rect(400, 50, 110, 30), OnlineMapsElevationManagerBase.instance.enabled, "Elevation");
        ...
        private void OnFindLocationComplete(string result)
        {
            ...
            if (searchMarker == null) searchMarker = OnlineMapsMarkerManager.CreateItem(position, search);
            else
            {
