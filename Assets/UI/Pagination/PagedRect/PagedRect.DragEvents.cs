...
namespace UI.Pagination
{
    public partial class PagedRect
    {
        ...
        public void OnBeginDrag(PointerEventData data)
        {
            ...
            m_DragStartX = data.position.x;
            m_DragEndX = data.position.x;
        }
        ...
        {            
            ...
            Debug.Log("Page drag startx: " + m_DragStartX + " endx: " + m_DragEndX);

            float difx = m_DragStartX - m_DragEndX;
            if (difx > 130.0f)
            {
                if (currentPageBeforeDragStarted < NumberOfPages)
                {
                    SetCurrentPage(currentPageBeforeDragStarted + 1);
                    return;
                }
                else if (LoopSeamlessly)
                {
                    SetCurrentPage(1);
                    return;
                }
            ...
            {
                if (currentPageBeforeDragStarted > 1)
                {
                    SetCurrentPage(currentPageBeforeDragStarted - 1);
                    return;
                }
            ...
            SetCurrentPage(currentPageBeforeDragStarted);
